...
// PLIC base discovered from firmware (DTB), QEMU virt fallback.
...
const PRIORITY_OFF: usize = 0x0000; // 4 bytes per IRQ
const ENABLE_OFF: usize = 0x2000; // 0x80 bytes per context
const THRESHOLD_OFF: usize = 0x20_0000; // 0x1000 per context
const CLAIM_OFF: usize = 0x20_0004; // 0x1000 per context
...
pub fn init(hart: u32) {
    ...
    let threshold = (base + THRESHOLD_OFF + ctx * 0x1000) as *mut u32;
    unsafe {
...
pub fn enable_irq(hart: u32, irq: u32) {
    ...
    let prio = (base + PRIORITY_OFF + 4 * irq as usize) as *mut u32;
    unsafe {
    ...
    let ctx = s_context(hart);
    let enable_reg = (base + ENABLE_OFF + ctx * 0x80 + (irq / 32) as usize * 4) as *mut u32;
    unsafe {
...
pub fn claim(hart: u32) -> u32 {
    let ctx = s_context(hart);
    let claim_reg = (plic_base() + CLAIM_OFF + ctx * 0x1000) as *const u32;
...
pub fn complete(hart: u32, irq: u32) {
    let ctx = s_context(hart);
    let complete_reg = (plic_base() + CLAIM_OFF + ctx * 0x1000) as *mut u32;
    unsafe {
