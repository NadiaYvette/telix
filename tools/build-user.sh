...
if [ "$ARCH" = "x86_64" ] && command -v gcc >/dev/null 2>&1; then
  ...
  xkb_symbols   { include "pc+us+inet(evdev)" };
    ...
    # Phase 176: glibc pthread_create + pthread_join + printf-in-child.
    # Tier-2 milestone for Xwayland (wlroots/libxkbcommon/libinput need
    # full pthread).  Static-PIE so we don't drag in ld.so dyn-link
    # concerns; this isolates pthread/futex/TLS behaviour in linux_srv.
    echo "Building glibc_pthread_hello (Phase 176)..."
    ...
    # #136 probe-α: direct clone3 without libpthread.  Isolates kernel
    # CLONE_THREAD plumbing from glibc NPTL bootstrap.  Statically linked
    # with glibc for _start but uses direct syscall() for clone3/write/exit.
    echo "Building clone3_test (#136 probe-α)..."
    if gcc -static-pie -fPIE -O2 -fno-stack-protector -s \
    ...
    if [ -f "$INITRAMFS_DIR/lib64/libXdmcp.so.6" ]; then
        ...
        if gcc -pie -fPIE -O2 -fno-stack-protector -s \
                ...
                -l:libXdmcp.so.6 \
                -Wl,-rpath,/lib64 2>&1; then
            echo "  libXdmcp_dyn_test: $(wc -c < "$BINDIR/libXdmcp_dyn_test") bytes"
        else
    ...
    if [ -f "$INITRAMFS_DIR/lib64/libpixman-1.so.0" ]; then
        ...
        if gcc -pie -fPIE -O2 -fno-stack-protector -s \
                ...
                -l:libpixman-1.so.0 \
                -Wl,-rpath,/lib64 2>&1; then
            echo "  libpixman_dyn_test: $(wc -c < "$BINDIR/libpixman_dyn_test") bytes"
        else
            echo "  WARNING: libpixman_dyn_test build failed"
        fi
    fi
    ...
    if [ -f "$INITRAMFS_DIR/lib64/libwayland-client.so.0" ]; then
        ...
        if gcc -pie -fPIE -O2 -fno-stack-protector -s \
                ...
                -Wl,-rpath,/lib64 2>&1; then
            echo "  libwayland_dyn_test: $(wc -c < "$BINDIR/libwayland_dyn_test") bytes"
        else
            echo "  WARNING: libwayland_dyn_test build failed"
        fi
    fi
    ...
    if [ -f "$INITRAMFS_DIR/lib64/libX11.so.6" ]; then
        ...
        if gcc -pie -fPIE -O2 -fno-stack-protector -s \
                ...
                -l:libX11.so.6 \
                -Wl,-rpath,/lib64 2>&1; then
            echo "  libX11_dyn_test: $(wc -c < "$BINDIR/libX11_dyn_test") bytes"
        else
    ...
    if [ -f "$INITRAMFS_DIR/lib64/libfreetype.so.6" ]; then
        ...
        if gcc -pie -fPIE -O2 -fno-stack-protector -s \
                ...
                -Wl,--as-needed \
                -Wl,-rpath,/lib64 2>&1; then
            echo "  libfont_dyn_test: $(wc -c < "$BINDIR/libfont_dyn_test") bytes"
        else
            echo "  WARNING: libfont_dyn_test build failed"
        fi
    fi
