    ...
    loop {
        ...
        match msg.tag {
            ...
            FS_READ => {
                ...
                if grant_va != 0 {
                    // Multi-block grant fill (cap MAX_BLOCKS_PER_REPLY) with
                    // a yield between blocks so the main port can drain
                    // concurrent callers and the kernel's 10 s
                    // CALL_REPLY_TIMEOUT doesn't fire on chained
                    // VFS_OPEN/FS_OPEN that go through ext_srv too.
                    // Each block is read with the cache_srv-style
                    // volatile u64 + Release fence + mfence pattern so
                    ...
                    while bytes_done < to_read && blocks_done < MAX_BLOCKS_PER_REPLY {
                        // Yield before each subsequent block so other
                        // callers' messages on this port can be picked up.
                        if blocks_done > 0 {
                            syscall::yield_now();
                        }
                        let cur_off = offset + bytes_done;
                    ...
                    // Inline path: single block, MAX_INLINE bytes max.
