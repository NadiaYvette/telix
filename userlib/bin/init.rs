...
fn main(_arg0: u64, _arg1: u64, _arg2: u64) {
    ...
    // waitpid spin and prevents the boot from ever reaching H13.
    if STEP_H_FOCUS_H13 {
        syscall::debug_puts(b"Phase 5h sched_stress: SKIPPED (FOCUS_H13)\n");
    } else {
    ...
    {
        // Spawn linux_srv if not already running.
        ...
        if linux_ok {
            ...
            if child == 0 {
                ...
                if p == 2 {
                    ...
                    unsafe {
                        ...
                        let pg: u64;
                        core::arch::asm!("int 0x80", inlateout("rax") __NR_MMAP => pg,
                        ...
                        let act_va = pg;
                        let oldact_va = pg + 64;
                        core::ptr::write_volatile(act_va as *mut u64, 0xDEAD);
                        core::ptr::write_volatile((act_va + 8) as *mut u64, 4);
                        core::ptr::write_volatile((act_va + 16) as *mut u64, 0xBEEF);
                        core::ptr::write_volatile((act_va + 24) as *mut u64, 0);
                        ...
                        // 1. rt_sigaction(SIGUSR1, act, oldact, 8)
                        ...
                        // 2. rt_sigaction(SIGUSR1, NULL, oldact, 8) — read back
                        ...
                        // 3. rt_sigprocmask(SIG_BLOCK, {SIGUSR1}, oldset, 8)
                        let set_va = pg + 128;
                        let oldset_va = pg + 192;
                        let mask_val: u64 = 1 << (SIGUSR1 - 1);
                        ...
                        // 4. rt_sigprocmask(SIG_SETMASK, NULL, oldset, 8) — read back mask
                ...
                if exit_code == 0 {
                    ...
                    syscall::debug_puts(b"Phase 145e rt_sigaction early: FAILED (timeout)\n");
                } else {
                    syscall::debug_puts(b"Phase 145e rt_sigaction early: FAILED (exit=");
                    let mut buf = [0u8; 10];
            ...
            syscall::debug_puts(b"Phase 145e rt_sigaction early: SKIPPED\n");
        }
    }
    ...
    syscall::debug_puts(b"  init: APFS write smoke test...\n");
    {
        ...
        if let Some(apfs_port) = apfs_port_opt {
            ...
            if smoke_ok {
                ...
                syscall::debug_puts(b"APFS write smoke test: FAILED\n");
            }
        } else {
  ...
  if rd_aspace != 0 {
        ...
        if let Some(xfs_port) = xfs_port_opt {
            ...
            let create_h = if let Some(r) = syscall::call(xfs_port, 0x2500, n0, n1, 9u64, 0) {
                if r.tag == 0x2501 {
                    ...
                    syscall::debug_puts(b"    xfs-smoke create: FAIL tag=");
                    print_num(r.tag);
                    if r.tag == 0x2F00 {
                        syscall::debug_puts(b" err=");
                        print_num(r.data[0]);
            ...
            if let Some((h, fs_aspace)) = create_h {
                // 2) FS_WRITE "Hello XFS W!"
                let wdata = b"Hello XFS W!";
                let dp = syscall::mmap_anon(0, 1, 1);
                if let Some(va) = dp {
                    unsafe {
                        for i in 0..wdata.len() {
                            core::ptr::write((va + i) as *mut u8, wdata[i]);
                    ...
                    if syscall::grant_pages(fs_aspace, va, gdst, 1, false) {
                        if let Some(wr) = syscall::call(xfs_port, 0x2600, h, wdata.len() as u64, gdst as u64, 0) {
                            if wr.tag == 0x2601 && wr.data[0] as usize == wdata.len() {
                                ...
                                syscall::debug_puts(b"    xfs-smoke write: FAIL\n");
                                smoke_ok = false;
                            }
                        } else {
                            smoke_ok = false;
                        ...
                        if r.tag == 0x2001 {
                            ...
                            if let Some(rr) = syscall::call(xfs_port, 0x2100, h2, 0, sz, 0) {
                                if rr.tag == 0x2101 {
                                    ...
                                    if br == expected.len() && &c[..br] == expected {
                                        syscall::debug_puts(
                                            b"    xfs-smoke read-back: OK\n",
                                        );
                                    } else {
                                        syscall::debug_puts(
                                            b"    xfs-smoke read-back: MISMATCH\n",
                                        );
                                        smoke_ok = false;
                                    }
                                } else {
                ...
                if smoke_ok {
                    ...
                    if let Some(r) = syscall::call(xfs_port, 0x2700, n0, n1, 9u64, 0) {
                        if r.tag == 0x2701 {
                            ...
                            syscall::debug_puts(b"    xfs-smoke delete: FAIL\n");
                            smoke_ok = false;
                        }
                    }
                }
            ...
            if smoke_ok {
                ...
                syscall::debug_puts(b"XFS write smoke test: FAILED\n");
            }
        } else {
    ...
    syscall::debug_puts(b"  init: XFS symlink/link/rename test...\n");
    {
        if let Some(xfs_port) = syscall::ns_lookup(b"xfs") {
            ...
            if ok {
                ...
                if let Some(r) = syscall::call(xfs_port, 0x2C00, sn0, sn1, 7u64, target_word) {
                    if r.tag == 0x2C01 {
                        ...
                        syscall::debug_puts(b"    xfs symlink: FAIL\n");
                        ok = false;
                    }
                } else { ok = false; }
            }
            ...
            if ok {
                ...
                if let Some(r) = syscall::call(xfs_port, 0x2C10, sn0, sn1, 7u64, 0) {
                    if r.tag == 0x2C11 {
                        ...
                        let mut got = [0u8; 8];
                        let w1 = r.data[1].to_le_bytes();
                        for i in 0..tlen.min(8) { got[i] = w1[i]; }
                        if tlen == 4 && &got[..4] == b"orig" {
                            syscall::debug_puts(b"    xfs readlink: OK\n");
                        } else {
                            syscall::debug_puts(b"    xfs readlink: MISMATCH\n");
                            ok = false;
                        }
                    } else {
                        syscall::debug_puts(b"    xfs readlink: FAIL\n");
                        ok = false;
                    }
                } else { ok = false; }
            }
            ...
            if ok {
                ...
                if let Some(r) = syscall::call(xfs_port, 0x2C20, ln0, ln1, 8u64, new_word) {
                    if r.tag == 0x2C21 {
                        ...
                        syscall::debug_puts(b"    xfs link: FAIL\n");
                        ok = false;
                    }
                } else { ok = false; }
            }
            ...
            if ok {
                ...
                if let Some(r) = syscall::call(xfs_port, 0x2C30, rn0, rn1, 8u64, new_word) {
                    if r.tag == 0x2C31 {
                        ...
                        syscall::debug_puts(b"    xfs rename: FAIL\n");
                        ok = false;
                    }
                } else { ok = false; }
            }
            ...
            // Cleanup: delete sym.txt, orig.txt, moved.tx.
            ...
            if ok {
                ...
                syscall::debug_puts(b"XFS symlink/link/rename: FAILED\n");
            }
        } else {
    ...
    syscall::debug_puts(b"  init: running capability test...\n");
    {
        ...
        if ct_tid != u64::MAX {
            ...
            syscall::set_quota(ct_tid, 0, 3);
            // Register cap_svc AFTER set_quota — cap_test retries ns_lookup
            // until this service exists, naturally synchronizing the quota.
            syscall::ns_register(b"cap_svc", svc_port); // max 3 ports
    ...
    syscall::debug_puts(b"  init: Phase 178 UDF filesystem...\n");
    {
        ...
        if udf_tid != u64::MAX {
            ...
            if let Some(udf_port) = udf_port {
                ...
                {
                    let (n0, n1, _) = syscall::pack_name(b"hello.txt");
                    if let Some(r) = syscall::call(udf_port, FS_OPEN, n0, n1, 9, 0) {
                        if r.tag == FS_OPEN_OK {
                            let handle = r.data[0];
                            if let Some(rr) = syscall::call(udf_port, FS_READ, handle, 0, 20, 0) {
                                if rr.tag == FS_READ_OK && rr.data[0] > 0 {
                                    let b0 = (rr.data[1] & 0xFF) as u8;
                                    let b1 = ((rr.data[1] >> 8) & 0xFF) as u8;
                                    let b2 = ((rr.data[1] >> 16) & 0xFF) as u8;
                                    ...
                                    if b0 == b'H' && b1 == b'e' && b2 == b'l' && b3 == b'l' && b4 == b'o' {
                                        ...
                                        syscall::debug_puts(b"  [178] hello.txt content mismatch\n");
                                        passed = false;
                                    }
                                } else {
                                    syscall::debug_puts(b"  [178] hello.txt read failed\n");
                                    passed = false;
                                }
                            } else { passed = false; }
                            let _ = syscall::call(udf_port, FS_CLOSE, handle, 0, 0, 0);
                        } else {
                            syscall::debug_puts(b"  [178] hello.txt open failed\n");
                            passed = false;
                        }
                    } else { passed = false; }
                }
                ...
                {
                    let (n0, n1, _) = syscall::pack_name(b"sub/deep.txt");
                    if let Some(r) = syscall::call(udf_port, FS_OPEN, n0, n1, 12, 0) {
                        if r.tag == FS_OPEN_OK {
                            let handle = r.data[0];
                            if let Some(rr) = syscall::call(udf_port, FS_READ, handle, 0, 19, 0) {
                                if rr.tag == FS_READ_OK && rr.data[0] > 0 {
                                    let b0 = (rr.data[1] & 0xFF) as u8;
                                    let b1 = ((rr.data[1] >> 8) & 0xFF) as u8;
                                    if b0 == b'N' && b1 == b'e' {
                                        ...
                                        syscall::debug_puts(b"  [178] deep.txt content mismatch\n");
                                        passed = false;
                                    }
                                } else { passed = false; }
                            } else { passed = false; }
                            ...
                            syscall::debug_puts(b"  [178] sub/deep.txt open failed\n");
                            passed = false;
                        }
                    } else { passed = false; }
                }
                ...
                {
                    let (n0, n1, _) = syscall::pack_name(b"/");
                    if let Some(r) = syscall::call(udf_port, FS_OPEN, n0, n1, 1, 0) {
                        if r.tag == FS_OPEN_OK {
                            let handle = r.data[0];
                            let mut entries = 0u32;
                            loop {
                                if let Some(rr) = syscall::call(udf_port, FS_READDIR, handle, 0, 0, 0) {
                                    if rr.tag == FS_READDIR_OK {
                                        entries += 1;
                                    } else { break; }
                                } else { break; }
                            ...
                            if entries >= 3 {
                                ...
                                syscall::debug_puts(b"  [178] readdir got ");
                                print_num(entries as u64);
                                syscall::debug_puts(b" entries (expected >=3)\n");
                                passed = false;
                            }
                ...
                // Test 4: Stat a file
                {
                    ...
                    if let Some(r) = syscall::call(udf_port, FS_OPEN, n0, n1, 9, 0) {
                        if r.tag == FS_OPEN_OK {
                            let handle = r.data[0];
                            if let Some(sr) = syscall::call(udf_port, FS_STAT, handle, 0, 0, 0) {
                                if sr.tag == FS_STAT_OK {
                                    let size = sr.data[0];
                                    let ftype = sr.data[1];
                                    if size == 26 && ftype == 0 {
                                        ...
                                        syscall::debug_puts(b"  [178] stat: size=");
                                        print_num(size);
                                        syscall::debug_puts(b" type=");
                                        print_num(ftype);
                                        syscall::debug_puts(b"\n");
                ...
                if passed && checks == 4 {
                    syscall::debug_puts(b"Phase 178 UDF filesystem: PASSED (");
                    print_num(checks as u64);
                    syscall::debug_puts(b"/4 checks)\n");
                } else {
                    syscall::debug_puts(b"Phase 178 UDF filesystem: FAILED (");
                    print_num(checks as u64);
                    syscall::debug_puts(b"/4 checks)\n");
                }
            } else {
                syscall::debug_puts(b"Phase 178 UDF filesystem: FAILED (udf not found)\n");
            }
        } else {
    ...
    // --- Phase 180: XFS write support ---
    syscall::debug_puts(b"  init: Phase 180 XFS write...\n");
    {
        ...
        if let Some(xfs_port) = xfs_port {
            ...
            let create_handle = if let Some(reply) = syscall::call(xfs_port, 0x2500, n0, n1, 10u64, 0) {
                if reply.tag == 0x2501 {
                    ...
                    print_num(h);
                    syscall::debug_puts(b"\n");
                    Some((h, reply.data[2])) // (handle, fs_aspace)
                } else {
                    syscall::debug_puts(b"    xfs create FAILED tag=");
                    print_num(reply.tag);
                    if reply.tag == 0x2F00 {
                        syscall::debug_puts(b" err=");
                        print_num(reply.data[0]);
                ...
                syscall::debug_puts(b"    xfs create no reply\n");
                write_ok = false;
                None
            };

            if let Some((handle, fs_aspace)) = create_handle {
                // Step 2: Write "XFS write OK!" to the file via FS_WRITE (0x2600).
                let write_data = b"XFS write OK!";
                let data_page = syscall::mmap_anon(0, 1, 1);
                if let Some(va) = data_page {
                    unsafe {
                        for i in 0..write_data.len() {
                            core::ptr::write((va + i) as *mut u8, write_data[i]);
                        }
                    }
                        ...
                        if let Some(reply) = syscall::call(
                            ...
                            if reply.tag == 0x2601 {
                                ...
                                if written == write_data.len() {
                                    syscall::debug_puts(b"    xfs write: OK (");
                                    print_num(written as u64);
                                    syscall::debug_puts(b" bytes)\n");
                                } else {
                                    syscall::debug_puts(b"    xfs write: short (");
                                    print_num(written as u64);
                                    syscall::debug_puts(b")\n");
                                    write_ok = false;
                                }
                            } else {
                                syscall::debug_puts(b"    xfs write FAILED tag=");
                                print_num(reply.tag);
                                if reply.tag == 0x2F00 {
                                    syscall::debug_puts(b" err=");
                                    print_num(reply.data[0]);
                            ...
                            syscall::debug_puts(b"    xfs write no reply\n");
                            write_ok = false;
                        }
                ...
                if write_ok {
                    ...
                    if let Some(reply) = syscall::call(xfs_port, 0x2000, n0, n1, 10u64, 0) {
                        if reply.tag == 0x2001 {
                            let h2 = reply.data[0];
                            let sz = reply.data[1];
                            syscall::debug_puts(b"    xfs reopen test_w.txt: size=");
                            print_num(sz);
                            syscall::debug_puts(b"\n");
                            ...
                            if let Some(rr) = syscall::call(xfs_port, 0x2100, h2, 0, sz, 0) {
                                if rr.tag == 0x2101 {
                                    ...
                                    {
                                        syscall::debug_puts(
                                            b"    xfs read-back: OK (\"XFS write OK!\")\n",
                                        );
                                    } else {
                                        syscall::debug_puts(
                                            b"    xfs read-back: content mismatch\n",
                                        );
                                        write_ok = false;
                                    }
                                } else {
                                    syscall::debug_puts(b"    xfs read-back FAILED\n");
                                    write_ok = false;
                                }
                            }
                ...
                if write_ok {
                    ...
                    if let Some(reply) = syscall::call(xfs_port, 0x2700, n0, n1, 10u64, 0) {
                        if reply.tag == 0x2701 {
                            ...
                            syscall::debug_puts(b"    xfs delete FAILED tag=");
                            print_num(reply.tag);
                            syscall::debug_puts(b"\n");
                            write_ok = false;
                        }
                    ...
                    if let Some(reply) = syscall::call(xfs_port, 0x2000, n0, n1, 10u64, 0) {
                        if reply.tag == 0x2F00 {
                            ...
                            syscall::debug_puts(
                                b"    xfs verify-gone: FAIL (file still exists)\n",
                            );
                            write_ok = false;
                        }
                    }
            ...
            if write_ok {
                ...
                syscall::debug_puts(b"Phase 180 XFS write: FAILED\n");
            }
        } else {
    ...
    syscall::debug_puts(b"  init: Phase 190 btrfs filesystem...\n");
    {
        match syscall::ns_lookup(b"btrfs") {
            ...
            None => {
                syscall::debug_puts(b"Phase 190 btrfs: SKIPPED (no btrfs_srv)\n");
            }
        }
    }
        ...
        if linux_ok && vfs_ok {
            ...
            if child == 0 {
                ...
                if exit_code == 0 {
                    ...
                    syscall::debug_puts(b"Step H libxcvt dyn-link (early): FAILED (timeout)\n");
                } else {
                    syscall::debug_puts(b"Step H libxcvt dyn-link (early): FAILED (exit=");
                    let mut buf = [0u8; 10];
                    let mut val = exit_code as u32;
                    let mut i = 10;
                    if val == 0 { i -= 1; buf[i] = b'0'; }
    ...
    syscall::debug_puts(b"  init: Step H8 libX11 dyn-link...\n");
    {
        ...
        if linux_ok && vfs_ok {
            ...
            if child == 0 {
                ...
                #[cfg(target_arch = "x86_64")]
                unsafe {
                    static PATH: &[u8] = b"/libX11_dyn_test\0";
                    static A0: &[u8] = b"libX11_dyn_test\0";
                    static E0: &[u8] = b"LD_LIBRARY_PATH=/lib64\0";
                    let argv: [u64; 2] = [A0.as_ptr() as u64, 0];
                    let envp: [u64; 2] = [E0.as_ptr() as u64, 0];
                    core::arch::asm!(
                ...
                if exit_code == 0 {
                    ...
                    syscall::debug_puts(b"Step H8 libX11 dyn-link: FAILED (timeout)\n");
                } else {
                    syscall::debug_puts(b"Step H8 libX11 dyn-link: FAILED (exit=");
                    let mut buf = [0u8; 10];
                    let mut val = exit_code as u32;
                    let mut i = 10;
                    if val == 0 { i -= 1; buf[i] = b'0'; }
    ...
    syscall::debug_puts(b"  init: Step H10 Tier-4 fonts batch...\n");
    {
        ...
        if linux_ok && vfs_ok {
            ...
            if child == 0 {
                ...
                #[cfg(target_arch = "x86_64")]
                unsafe {
                    static PATH: &[u8] = b"/libfont_dyn_test\0";
                    static A0: &[u8] = b"libfont_dyn_test\0";
                    static E0: &[u8] = b"LD_LIBRARY_PATH=/lib64\0";
                    let argv: [u64; 2] = [A0.as_ptr() as u64, 0];
                    let envp: [u64; 2] = [E0.as_ptr() as u64, 0];
                    core::arch::asm!(
                ...
                if exit_code == 0 {
                    ...
                    syscall::debug_puts(b"Step H10 Tier-4 fonts batch: FAILED (timeout)\n");
                } else {
                    syscall::debug_puts(b"Step H10 Tier-4 fonts batch: FAILED (exit=");
                    let mut buf = [0u8; 10];
                    let mut val = exit_code as u32;
                    let mut i = 10;
                    if val == 0 { i -= 1; buf[i] = b'0'; }
    ...
    syscall::debug_puts(b"  init: Step H11 Tier-5 SSL batch...\n");
    {
        ...
        if linux_ok && vfs_ok {
            ...
            if child == 0 {
                ...
                #[cfg(target_arch = "x86_64")]
                unsafe {
                    static PATH: &[u8] = b"/libssl_dyn_test\0";
                    static A0: &[u8] = b"libssl_dyn_test\0";
                    static E0: &[u8] = b"LD_LIBRARY_PATH=/lib64\0";
                    let argv: [u64; 2] = [A0.as_ptr() as u64, 0];
                    let envp: [u64; 2] = [E0.as_ptr() as u64, 0];
                    core::arch::asm!(
                ...
                if exit_code == 0 {
                    ...
                    syscall::debug_puts(b"Step H11 Tier-5 SSL batch: FAILED (timeout)\n");
                } else {
                    syscall::debug_puts(b"Step H11 Tier-5 SSL batch: FAILED (exit=");
                    let mut buf = [0u8; 10];
                    let mut val = exit_code as u32;
                    let mut i = 10;
                    if val == 0 { i -= 1; buf[i] = b'0'; }
    ...
    // the *full* dyn-link chain — Xwayland has 25+ DT_NEEDED entries
    // covering Tiers 0-5 plus Mesa (libGL/libgbm), libei/liboeffis,
        ...
        if linux_ok && vfs_ok {
            ...
            if child == 0 {
                ...
                if exit_code == 0 {
                    ...
                    syscall::debug_puts(b"Step H12 Xwayland binary load: FAILED (timeout)\n");
                } else {
                    syscall::debug_puts(b"Step H12 Xwayland binary load: FAILED (exit=");
                    let mut buf = [0u8; 10];
                    let mut val = exit_code as u32;
                    let mut i = 10;
                    if val == 0 { i -= 1; buf[i] = b'0'; }
                ...
                if xw_child == 0 {
                    ...
                    if xeyes_child != u64::MAX && xeyes_attempt_reported < xeyes_attempt {
                        ...
                        if xeyes_code >= 0 {
                            ...
                            syscall::debug_puts(b"never_reaped");
                        }
                        syscall::debug_puts(b"\n");
                    }
    ...
    syscall::debug_puts(b"  init: Step H2 libdrm dyn-link...\n");
    {
        ...
        if linux_ok && vfs_ok {
            ...
            if child == 0 {
                ...
                #[cfg(target_arch = "x86_64")]
                unsafe {
                    static PATH: &[u8] = b"/libdrm_dyn_test\0";
                    static A0: &[u8] = b"libdrm_dyn_test\0";
                    static E0: &[u8] = b"LD_LIBRARY_PATH=/lib64\0";
                    let argv: [u64; 2] = [A0.as_ptr() as u64, 0];
                    let envp: [u64; 2] = [E0.as_ptr() as u64, 0];
                    core::arch::asm!(
                ...
                if exit_code == 0 {
                    ...
                    syscall::debug_puts(b"Step H2 libdrm dyn-link: FAILED (timeout)\n");
                } else {
                    syscall::debug_puts(b"Step H2 libdrm dyn-link: FAILED (exit=");
                    let mut buf = [0u8; 10];
                    let mut val = exit_code as u32;
                    let mut i = 10;
                    if val == 0 { i -= 1; buf[i] = b'0'; }
    ...
    // Exercises alloc → map → unmap path which depends on memfd_create
    // + ftruncate + fcntl(F_ADD_SEALS) (Phase 46 + memfd sealing).
    syscall::debug_puts(b"  init: Step H3 libxshmfence dyn-link...\n");
    {
        let linux_ok = syscall::ns_lookup(b"linux").is_some();
        let vfs_ok   = syscall::ns_lookup(b"vfs").is_some();
        if linux_ok && vfs_ok {
            ...
            if child == 0 {
                ...
                #[cfg(target_arch = "x86_64")]
                unsafe {
                    static PATH: &[u8] = b"/libxshmfence_dyn_test\0";
                    static A0: &[u8] = b"libxshmfence_dyn_test\0";
                    static E0: &[u8] = b"LD_LIBRARY_PATH=/lib64\0";
                    let argv: [u64; 2] = [A0.as_ptr() as u64, 0];
                    let envp: [u64; 2] = [E0.as_ptr() as u64, 0];
                    core::arch::asm!(
                ...
                if exit_code == 0 {
                    ...
                    syscall::debug_puts(b"Step H3 libxshmfence dyn-link: FAILED (timeout)\n");
                } else {
                    syscall::debug_puts(b"Step H3 libxshmfence dyn-link: FAILED (exit=");
                    let mut buf = [0u8; 10];
                    let mut val = exit_code as u32;
                    let mut i = 10;
                    if val == 0 { i -= 1; buf[i] = b'0'; }
            ...
            syscall::debug_puts(b"Step H3 libxshmfence dyn-link: SKIPPED\n");
        }
    }

    // --- Step H4: libXau.so.6 dyn-link smoke (Tier-1 cookie-auth) ---
    // First Tier-1 leaf.  XauFileName reads $XAUTHORITY/$HOME via
    // getenv + malloc, no fs access — safe on Telix without an
    // .Xauthority file present.  PASS == exit 0 regardless of the
    // returned path (NULL is the expected outcome here).
    syscall::debug_puts(b"  init: Step H4 libXau dyn-link...\n");
    {
        let linux_ok = syscall::ns_lookup(b"linux").is_some();
        let vfs_ok   = syscall::ns_lookup(b"vfs").is_some();
        if linux_ok && vfs_ok {
            ...
            if child == 0 {
                ...
                #[cfg(target_arch = "x86_64")]
                unsafe {
                    static PATH: &[u8] = b"/libXau_dyn_test\0";
                    static A0: &[u8] = b"libXau_dyn_test\0";
                    static E0: &[u8] = b"LD_LIBRARY_PATH=/lib64\0";
                    let argv: [u64; 2] = [A0.as_ptr() as u64, 0];
                    let envp: [u64; 2] = [E0.as_ptr() as u64, 0];
                    core::arch::asm!(
                ...
                if exit_code == 0 {
                    ...
                    syscall::debug_puts(b"Step H4 libXau dyn-link: FAILED (timeout)\n");
                } else {
                    syscall::debug_puts(b"Step H4 libXau dyn-link: FAILED (exit=");
                    let mut buf = [0u8; 10];
                    let mut val = exit_code as u32;
                    let mut i = 10;
                    if val == 0 { i -= 1; buf[i] = b'0'; }
            ...
            syscall::debug_puts(b"Step H4 libXau dyn-link: SKIPPED\n");
        }
    }
    ...
    // --- Step H: Tier-0 Xwayland porting — dyn link to Fedora libxcvt.so.0 ---
    //
    // Runs /libxcvt_dyn_test under the Linux personality.  The binary is
    // a tiny dynamically-linked PIE that calls libxcvt_gen_mode_info() to
    // verify ld.so can load Fedora's libxcvt.so.0 on top of libc.so.6.
    // Placed after Phase 172e (known-good dyn-link baseline) so a
    // regression in Step H points at the extra .so, not at ld.so itself.
    // Requires VFS, so sits after Phase 51.  PASS == exit 0.
    syscall::debug_puts(b"  init: Step H libxcvt dyn-link...\n");
    {
        let linux_ok = syscall::ns_lookup(b"linux").is_some();
        let vfs_ok   = syscall::ns_lookup(b"vfs").is_some();
        if linux_ok && vfs_ok {
            ...
            if child == 0 {
                ...
                if exit_code == 0 {
                    ...
                    syscall::debug_puts(b"Step H libxcvt dyn-link: FAILED (timeout)\n");
                } else {
                    syscall::debug_puts(b"Step H libxcvt dyn-link: FAILED (exit=");
                    let mut buf = [0u8; 10];
                    let mut val = exit_code as u32;
                    let mut i = 10;
                    if val == 0 { i -= 1; buf[i] = b'0'; }
            ...
            syscall::debug_puts(b"Step H libxcvt dyn-link: SKIPPED\n");
        }
    }
    ...
    syscall::debug_puts(b"  init: testing devfs...\n");
    {
        ...
        if phase55_ok {
            ...
            let (fn0, fn1, _) = pack_name(fname);
            let d2 = fname.len() as u64;
            if let Some(reply) = syscall::call(devfs_port, 0x2000, fn0, fn1, d2, 0) {
                if reply.tag == 0x2001 {
                    let h = reply.data[0];

                    // Write 16 bytes — should succeed (discard).
                    let wd1 = 16u64;
                    if let Some(wr) = syscall::call(devfs_port, 0x2600, h, wd1, 0, 0) {
                        if wr.tag != 0x2601 {
                    ...
                    if phase55_ok {
                        let rd2 = 8u64;
                        if let Some(rr) = syscall::call(devfs_port, 0x2100, h, 0, rd2, 0) {
                            if rr.tag == 0x2101 {
                                if rr.data[0] != 0 {
        ...
        if phase55_ok {
            ...
            for _ in 0..20 {
                if let Some(rr) = syscall::call(devfs_port, 0x2200, next_off, 0, 0, 0) {
                    if rr.tag == 0x2201 {
                        count += 1;
                        next_off = rr.data[3];
                    } else {
    ...
    // --- Phase 116: Wallpaper gradient + taskbar clock ---
    syscall::debug_puts(b"  init: Phase 116 wallpaper gradient + clock...\n");
    {
        let comp_ok = syscall::ns_lookup(b"compositor").is_some();
        if comp_ok {
            syscall::debug_puts(b"Phase 116 wallpaper gradient + clock: PASSED\n");
        } else {
            syscall::debug_puts(b"Phase 116 wallpaper gradient + clock: FAILED\n");
        }
    }

    // --- Phase 117: Cascade window placement ---
    syscall::debug_puts(b"  init: Phase 117 cascade placement...\n");
    {
        let comp_ok = syscall::ns_lookup(b"compositor").is_some();
        if comp_ok {
            syscall::debug_puts(b"Phase 117 cascade placement: PASSED\n");
        } else {
            syscall::debug_puts(b"Phase 117 cascade placement: FAILED\n");
        }
    }
    ...
    // --- Phase 132: Linux memfd_create ---
    syscall::debug_puts(b"  init: Phase 132 linux memfd_create...\n");
    {
        let linux_ok = syscall::ns_lookup(b"linux").is_some();
        if linux_ok {
            let child = syscall::fork();
            if child == 0 {
                ...
                if p == 2 {
                    ...
                    unsafe {
                        ...
                        // 2. write(mfd, "Hello memfd!", 12) — NR 1
                        //    (write auto-grows backing memory)
                        let msg = *b"Hello memfd!";
                        let wr = linux!(1u64, mfd, msg.as_ptr() as u64, 12u64);
                        if wr != 12 {
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 12u64, options(noreturn));
                        }

                        // 3. lseek(mfd, 0, SEEK_SET=0) — NR 8
                        let off = linux!(8u64, mfd, 0u64, 0u64);
                        if off != 0 {
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 13u64, options(noreturn));
                        }

                        // 4. read(mfd, buf, 12) — NR 0
                        let mut rbuf = [0u8; 12];
                        let rd = linux!(0u64, mfd, rbuf.as_mut_ptr() as u64, 12u64);
                        if rd != 12 {
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 14u64, options(noreturn));
                        }
                        // Volatile read to defeat compiler optimization.
                        let mut ok = true;
                        for i in 0..12 {
                            if core::ptr::read_volatile(&rbuf[i]) != msg[i] {
                                ok = false;
                                break;
                            }
                        }
                        ...
                        // 5. ftruncate(mfd, 64) — NR 77 (test separately)
                ...
                if exit_code == 0 {
                    ...
                    syscall::debug_puts(b"Phase 132 linux memfd_create: FAILED (timeout)\n");
                } else {
                    syscall::debug_puts(b"Phase 132 linux memfd_create: FAILED (exit=");
                    let mut buf = [0u8; 10];
                    let mut val = exit_code as u32;
                    let mut i = 10;
                    if val == 0 { i -= 1; buf[i] = b'0'; }
            ...
            syscall::debug_puts(b"Phase 132 linux memfd_create: SKIPPED\n");
        }
    }
    ...
    // --- Phase 135: 6-arg personality routing ---
    // The kernel now packs caller_port into msg.tag (high 32 bits) and forwards
    // all 6 syscall args in data[0..5]. Test by using mmap (6 args: addr, len,
    // prot, flags, fd, offset) and verifying the mapping works.
    syscall::debug_puts(b"  init: Phase 135 linux 6-arg routing...\n");
    {
        let linux_ok = syscall::ns_lookup(b"linux").is_some();
        if linux_ok {
            let child = syscall::fork();
            if child == 0 {
                ...
                if p == 2 {
                    ...
                    unsafe {
                        ...
                        // Test 1: prlimit64 — verifies tag packing (caller_port in high bits)
                        let mut rlim = [0u8; 16];
                        let r = linux!(302u64, 0u64, 7u64, 0u64, rlim.as_mut_ptr() as u64);
                        if r > 0xFFFF_FFFF_FFFF_FF00 {
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 10u64, options(noreturn));
                        ...
                        // Test 2: sendmsg with single iovec — verifies multi-arg routing
                        // Create socketpair
                        let mut sv: [i32; 2] = [0; 2];
                        let r = linux!(53u64, 1u64, 1u64, 0u64, sv.as_mut_ptr() as u64);
                        if r > 0xFFFF_FFFF_FFFF_FF00 {
                        ...
                        // write "test" on s0, read on s1
                        let msg = *b"test";
                        let wr: u64;
                        core::arch::asm!("int 0x80", inlateout("rax") 1u64 => wr,
                            in("rdi") s0, in("rsi") msg.as_ptr() as u64, in("rdx") 4u64,
                            in("r10") 0u64, in("r8") 0u64,
                            lateout("rcx") _, lateout("r11") _);
                        if wr != 4 {
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 13u64, options(noreturn));
                        ...
                        let mut rbuf = [0u8; 4];
                        let rd: u64;
                        core::arch::asm!("int 0x80", inlateout("rax") 0u64 => rd,
                            in("rdi") s1, in("rsi") rbuf.as_mut_ptr() as u64, in("rdx") 4u64,
                            in("r10") 0u64, in("r8") 0u64,
                            lateout("rcx") _, lateout("r11") _);
                        if rd != 4 {
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 14u64, options(noreturn));
                ...
                if exit_code == 0 {
                    ...
                    syscall::debug_puts(b"Phase 135 linux 6-arg routing: FAILED (timeout)\n");
                } else {
                    syscall::debug_puts(b"Phase 135 linux 6-arg routing: FAILED (exit=");
                    let mut buf = [0u8; 10];
                    let mut val = exit_code as u32;
                    let mut i = 10;
                    if val == 0 { i -= 1; buf[i] = b'0'; }
            ...
            syscall::debug_puts(b"Phase 135 linux 6-arg routing: SKIPPED\n");
        }
    }
    ...
    // --- Phase 137: getcwd fix, readlinkat, faccessat, umask ---
    syscall::debug_puts(b"  init: Phase 137 linux metadata syscalls...\n");
    {
        let linux_ok = syscall::ns_lookup(b"linux").is_some();
        if linux_ok {
            let child = syscall::fork();
            if child == 0 {
                ...
                if p == 2 {
                    ...
                    unsafe {
                        ...
                        // 1. umask(0o077) — NR 95 — should return old mask (0o022)
                        let old = linux!(95u64, 0o077u64);
                        if old != 0o022 {
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 10u64, options(noreturn));
                        }

                        // 2. umask(0o022) — should return 0o077 (round-trip)
                        let old2 = linux!(95u64, 0o022u64);
                        if old2 != 0o077 {
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 11u64, options(noreturn));
                        }

                        // 3. faccessat(AT_FDCWD=-100, "/", F_OK=0, 0) — NR 269
                        let root = *b"/\0";
                        let r = linux!(269u64, (-100i64) as u64, root.as_ptr() as u64, 0u64, 0u64);
                        if r > 0xFFFF_FFFF_FFFF_FF00 {
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 12u64, options(noreturn));
                        }

                        // 4. readlinkat(AT_FDCWD, "/proc/self/exe", buf, 64) — NR 267
                        let procexe = *b"/proc/self/exe\0";
                        let mut linkbuf = [0u8; 64];
                        let rlen = linux!(267u64, (-100i64) as u64, procexe.as_ptr() as u64,
                                          linkbuf.as_mut_ptr() as u64, 64u64);
                        if rlen == 0 || rlen > 0xFFFF_FFFF_FFFF_FF00 {
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 13u64, options(noreturn));
                        }

                        // 5. getcwd(buf, 64) — NR 79 — should return "/"
                        let mut cwdbuf = [0u8; 64];
                        let cwd_r = linux!(79u64, cwdbuf.as_mut_ptr() as u64, 64u64);
                        if cwd_r > 0xFFFF_FFFF_FFFF_FF00 {
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 14u64, options(noreturn));
                        }
                        // Verify CWD starts with '/'
                        if core::ptr::read_volatile(&cwdbuf[0]) != b'/' {
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 15u64, options(noreturn));
                        }
                ...
                if exit_code == 0 {
                    ...
                    syscall::debug_puts(b"Phase 137 linux metadata syscalls: FAILED (timeout)\n");
                } else {
                    syscall::debug_puts(b"Phase 137 linux metadata syscalls: FAILED (exit=");
                    let mut buf = [0u8; 10];
                    let mut val = exit_code as u32;
                    let mut i = 10;
                    if val == 0 { i -= 1; buf[i] = b'0'; }
            ...
            syscall::debug_puts(b"Phase 137 linux metadata syscalls: SKIPPED\n");
        }
    }
    ...
    // --- Phase 140: kill, tgkill, flock stubs ---
    // NOTE: mmap/mremap via personality maps in linux_srv's address space,
    // not the caller's.  A future phase will add SYS_PERSONALITY_MMAP.
    syscall::debug_puts(b"  init: Phase 140 linux kill/flock...\n");
    {
        let linux_ok = syscall::ns_lookup(b"linux").is_some();
        if linux_ok {
            let child = syscall::fork();
            if child == 0 {
                ...
                if p == 2 {
                    ...
                    unsafe {
                        ...
                        // 1. kill(getpid(), 0) -- NR 62 -- signal 0 existence check
                        let pid = linux!(39u64); // getpid
                        let kr = linux!(62u64, pid, 0u64);
                        if kr > 0xFFFF_FFFF_FFFF_FF00 {
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 10u64, options(noreturn));
                        }

                        // 2. kill(99999, 0) -- should return ESRCH (-3)
                        let kr2 = linux!(62u64, 99999u64, 0u64);
                        // Expect negative error (high bits set)
                        if kr2 < 0xFFFF_FFFF_FFFF_FF00 {
                            // Should have been an error
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 11u64, options(noreturn));
                        }

                        // 3. flock(1, LOCK_SH) -- NR 73 -- stub returns 0
                        let fl = linux!(73u64, 1u64, 1u64); // LOCK_SH=1 on fd 1
                        if fl > 0xFFFF_FFFF_FFFF_FF00 {
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 12u64, options(noreturn));
                        }

                        // 4. sigaltstack(NULL, NULL) -- NR 131 -- stub returns 0
                        let sa = linux!(131u64, 0u64, 0u64);
                        if sa > 0xFFFF_FFFF_FFFF_FF00 {
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 13u64, options(noreturn));
                        }

                        // 5. fchmod(1, 0644) -- NR 91 -- stub returns 0
                        let fc = linux!(91u64, 1u64, 0o644u64);
                        if fc > 0xFFFF_FFFF_FFFF_FF00 {
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 14u64, options(noreturn));
                        }
                ...
                if exit_code == 0 {
                    ...
                    syscall::debug_puts(b"Phase 140 linux kill/flock: FAILED (timeout)\n");
                } else {
                    syscall::debug_puts(b"Phase 140 linux kill/flock: FAILED (exit=");
                    let mut buf = [0u8; 10];
                    let mut val = exit_code as u32;
                    let mut i = 10;
                    if val == 0 { i -= 1; buf[i] = b'0'; }
            ...
            syscall::debug_puts(b"Phase 140 linux kill/flock: SKIPPED\n");
        }
    }

    // --- Phase 141: cross-address-space mmap/mremap/brk ---
    syscall::debug_puts(b"  init: Phase 141 linux mmap/mremap/brk...\n");
    syscall::debug_puts(b"Phase 141 linux mmap/mremap/brk: SKIPPED (revisit during Tier 1)\n");
    if false {
        let linux_ok = syscall::ns_lookup(b"linux").is_some();
        if linux_ok {
            let child = syscall::fork();
            if child == 0 {
                ...
                if p == 2 {
                    ...
                    unsafe {
                        ...
                        // 1. mmap anonymous page
                        let addr = linux!(9u64, 0u64, page_sz, 3u64, 0x22u64);
                        if addr > 0xFFFF_FFFF_FFFF_F000 {
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 10u64, options(noreturn));
                        }
                        // 2. write marker + verify readback
                        core::ptr::write_volatile(addr as *mut u8, 0xAB);
                        if core::ptr::read_volatile(addr as *const u8) != 0xAB {
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 11u64, options(noreturn));
                        }
                        // 3. mremap: expand 4K → 8K (MREMAP_MAYMOVE=1)
                        let new_addr = linux!(25u64, addr, page_sz, 2u64 * page_sz, 1u64);
                        if new_addr > 0xFFFF_FFFF_FFFF_F000 {
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 12u64, options(noreturn));
                        }
                        // 4. verify marker survived mremap
                        if core::ptr::read_volatile(new_addr as *const u8) != 0xAB {
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 13u64, options(noreturn));
                        }
                        // 5. munmap the remapped region
                        let mun = linux!(11u64, new_addr, 2u64 * page_sz);
                        if mun != 0 {
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 14u64, options(noreturn));
                        }
                        // 6. brk(0) → get current brk
                        let brk0 = linux!(12u64, 0u64);
                        // 7. brk(+4K) → extend heap
                        let brk1 = linux!(12u64, brk0 + page_sz);
                        if brk1 <= brk0 {
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 15u64, options(noreturn));
                        }
                        // 8. write to brk area + verify
                        core::ptr::write_volatile(brk0 as *mut u8, 0xCD);
                        if core::ptr::read_volatile(brk0 as *const u8) != 0xCD {
                            core::arch::asm!("int 0x80", in("rax") 231u64, in("rdi") 16u64, options(noreturn));
                        }
                ...
                if exit_code == 0 {
                    ...
                    syscall::debug_puts(b"Phase 141 linux mmap/mremap/brk: FAILED (timeout)\n");
                } else {
                    syscall::debug_puts(b"Phase 141 linux mmap/mremap/brk: FAILED (exit=");
                    let mut buf = [0u8; 10];
                    let mut val = exit_code as u32;
                    let mut i = 10;
                    if val == 0 { i -= 1; buf[i] = b'0'; }
            ...
            syscall::debug_puts(b"Phase 141 linux mmap/mremap/brk: SKIPPED\n");
        }
    }
    ...
    // --- Phase 151: fsync/fallocate/utimensat/clone3 stubs ---
    syscall::debug_puts(b"  init: Phase 151 linux misc stubs...\n");
    {
        let linux_ok = syscall::ns_lookup(b"linux").is_some();
        if linux_ok {
            let child = syscall::fork();
            if child == 0 {
                ...
                if p == 2 {
                    ...
                    unsafe {
                        ...
                        // fsync on stdout (fd 1) — should return 0, not ENOSYS
                        let r = linux!(__NR_FSYNC, 1u64, 0u64, 0u64);
                        if r == ENOSYS_NEG {
                            core::arch::asm!("int 0x80", in("rax") __NR_EXIT_GROUP, in("rdi") 91u64, options(noreturn));
                        }

                        // fdatasync on stdout — should return 0
                        let r = linux!(__NR_FDATASYNC, 1u64, 0u64, 0u64);
                        if r == ENOSYS_NEG {
                            core::arch::asm!("int 0x80", in("rax") __NR_EXIT_GROUP, in("rdi") 92u64, options(noreturn));
                        }

                        // utimensat — should return 0
                        let r = linux!(__NR_UTIMENSAT, 0u64, 0u64, 0u64);
                        if r == ENOSYS_NEG {
                            core::arch::asm!("int 0x80", in("rax") __NR_EXIT_GROUP, in("rdi") 93u64, options(noreturn));
                        }

                        // All stubs handled correctly
                        core::arch::asm!("int 0x80", in("rax") __NR_EXIT_GROUP, in("rdi") 0u64, options(noreturn));
                    }
                    #[cfg(not(target_arch = "x86_64"))]
                    {
                ...
                if exit_code == 0 {
                    ...
                    syscall::debug_puts(b"Phase 151 linux misc stubs: FAILED (timeout)\n");
                } else {
                    syscall::debug_puts(b"Phase 151 linux misc stubs: FAILED (exit=");
                    let mut buf = [0u8; 10];
                    let mut val = exit_code as u32;
                    let mut i = 10;
                    if val == 0 { i -= 1; buf[i] = b'0'; }
            ...
            syscall::debug_puts(b"Phase 151 linux misc stubs: SKIPPED\n");
        }
    }
    ...
    syscall::debug_puts(b"  init: Phase 153 linux xattr/inotify stubs...\n");
    {
        let linux_ok = syscall::ns_lookup(b"linux").is_some();
        if linux_ok {
            let child = syscall::fork();
            if child == 0 {
                ...
                if p == 2 {
                    ...
                    unsafe {
                        ...
                        const __NR_EXIT_GROUP: u64 = 231;
                        const ENOSYS_NEG: u64 = (-38i64) as u64;
                        const ENODATA_NEG: u64 = (-61i64) as u64;

                        // listxattr should return 0 (empty list), not ENOSYS
                        let r = linux!(__NR_LISTXATTR, 0u64, 0u64, 0u64);
                        if r == ENOSYS_NEG {
                            core::arch::asm!("int 0x80", in("rax") __NR_EXIT_GROUP, in("rdi") 91u64, options(noreturn));
                        }
                        ...
                        // getxattr should return -ENODATA (61), not -ENOSYS
                        let r = linux!(__NR_GETXATTR, 0u64, 0u64, 0u64);
                        if r == ENOSYS_NEG {
                            core::arch::asm!("int 0x80", in("rax") __NR_EXIT_GROUP, in("rdi") 93u64, options(noreturn));
                        }
                        if r != ENODATA_NEG {
                ...
                if exit_code == 0 {
                    ...
                    syscall::debug_puts(b"Phase 153 linux xattr/inotify stubs: FAILED (timeout)\n");
                } else {
                    syscall::debug_puts(b"Phase 153 linux xattr/inotify stubs: FAILED (exit=");
                    let mut buf = [0u8; 10];
                    let mut val = exit_code as u32;
                    let mut i = 10;
                    if val == 0 { i -= 1; buf[i] = b'0'; }
            ...
            syscall::debug_puts(b"Phase 153 linux xattr/inotify stubs: SKIPPED\n");
        }
    }
    ...
    // --- Phase 159: /dev/null, /dev/zero, /dev/urandom virtual devices ---
    syscall::debug_puts(b"  init: Phase 159 linux virtual devices...\n");
    {
        let linux_ok = syscall::ns_lookup(b"linux").is_some();
        if linux_ok {
            let child = syscall::fork();
            if child == 0 {
                ...
                if p == 2 {
                    ...
                    unsafe {
                        ...
                        // Test 1: open /dev/null, write to it, close.
                        let path_null = b"/dev/null\0";
                        let fd_null: u64;
                        core::arch::asm!("int 0x80", inlateout("rax") __NR_OPEN => fd_null,
                            in("rdi") path_null.as_ptr() as u64,
                        ...
                        let wr: u64;
                        core::arch::asm!("int 0x80", inlateout("rax") __NR_WRITE => wr,
                            in("rdi") fd_null,
                            in("rsi") test_data.as_ptr() as u64,
                            in("rdx") 5u64,
                            lateout("rcx") _, lateout("r11") _);
                        if wr != 5 {
                            core::arch::asm!("int 0x80", in("rax") __NR_EXIT_GROUP, in("rdi") 92u64, options(noreturn));
                        ...
                        core::arch::asm!("int 0x80", inlateout("rax") __NR_CLOSE => _,
                            in("rdi") fd_null, lateout("rcx") _, lateout("r11") _);

                        // Test 2: open /dev/zero, read from it, verify zeros.
                        let path_zero = b"/dev/zero\0";
                        let fd_zero: u64;
                        core::arch::asm!("int 0x80", inlateout("rax") __NR_OPEN => fd_zero,
                            in("rdi") path_zero.as_ptr() as u64,
                        ...
                        core::arch::asm!("int 0x80", inlateout("rax") __NR_CLOSE => _,
                            in("rdi") fd_zero, lateout("rcx") _, lateout("r11") _);

                        // Test 3: open /dev/urandom, read from it, verify not all zeros.
                        let path_urand = b"/dev/urandom\0";
                        let fd_urand: u64;
                        core::arch::asm!("int 0x80", inlateout("rax") __NR_OPEN => fd_urand,
                            in("rdi") path_urand.as_ptr() as u64,
                            in("rsi") 0u64, // O_RDONLY
                            lateout("rcx") _, lateout("r11") _);
                        if (fd_urand as i64) < 0 {
                            core::arch::asm!("int 0x80", in("rax") __NR_EXIT_GROUP, in("rdi") 96u64, options(noreturn));
                        }
                        let mut rbuf = [0u8; 32];
                        let rd2: u64;
                        ...
                        // At least one byte should be non-zero (probability of all-zero is ~2^-256).
                        let mut any_nonzero = false;
                        for i in 0..32 {
                            if rbuf[i] != 0 { any_nonzero = true; }
                        }
                        if !any_nonzero {
                            core::arch::asm!("int 0x80", in("rax") __NR_EXIT_GROUP, in("rdi") 98u64, options(noreturn));
                        }

                        // Test 4: fstat /dev/urandom — should be char device.
                        let mut stat_buf = [0u8; 144];
                        let fs: u64;
                        core::arch::asm!("int 0x80", inlateout("rax") __NR_FSTAT => fs,
                            in("rdi") fd_urand,
                            in("rsi") stat_buf.as_mut_ptr() as u64,
                            lateout("rcx") _, lateout("r11") _);
                        if (fs as i64) < 0 {
                            core::arch::asm!("int 0x80", in("rax") __NR_EXIT_GROUP, in("rdi") 99u64, options(noreturn));
                        }
                        // st_mode at offset 24: should have S_IFCHR (0o020000) bit set.
                        let mode = u32::from_le_bytes([stat_buf[24], stat_buf[25], stat_buf[26], stat_buf[27]]);
                        if mode & 0o170000 != 0o020000 {
                            core::arch::asm!("int 0x80", in("rax") __NR_EXIT_GROUP, in("rdi") 100u64, options(noreturn));
                        }

                        core::arch::asm!("int 0x80", inlateout("rax") __NR_CLOSE => _,
                            in("rdi") fd_urand, lateout("rcx") _, lateout("r11") _);
                ...
                if exit_code == 0 {
                    ...
                    syscall::debug_puts(b"Phase 159 linux virtual devices: FAILED (timeout)\n");
                } else {
                    syscall::debug_puts(b"Phase 159 linux virtual devices: FAILED (exit=");
                    let mut buf = [0u8; 10];
                    let mut val = exit_code as u32;
                    let mut i = 10;
                    if val == 0 { i -= 1; buf[i] = b'0'; }
            ...
            syscall::debug_puts(b"Phase 159 linux virtual devices: SKIPPED\n");
        }
    }
    ...
    // --- Phase 172: real ld-linux + libc.so.6 (dynamic-PIE) ---
    //
    // Same shape as Phase 171 but execve()s a real PIE ELF whose PT_INTERP
    // is /lib64/ld-linux-x86-64.so.2.  This exercises the kernel ELF loader's
    // PT_INTERP path, the long-path VFS protocol (ld.so opens libc.so.6 from
    // a 27-byte path), and linux_srv mmap-fixed for ld.so segment placement.
    // glibc_dyn_hello.c does `return argc;`, so success is exit code 4.
    syscall::debug_puts(b"  init: Phase 172 dynamic glibc binary...\n");
    {
        let linux_ok = syscall::ns_lookup(b"linux").is_some();
        if linux_ok {
            syscall::debug_puts(b"  [172] forking child\n");
            let child = syscall::fork();
            if child == 0 {
                for _ in 0..100 {
                    let (p, _) = syscall::personality_get();
                ...
                let (p, _) = syscall::personality_get();
                if p == 2 {
                    syscall::debug_puts(b"  [172 child] personality set, execve...\n");
                    #[cfg(target_arch = "x86_64")]
                    unsafe {
                        static PATH: &[u8] = b"/glibc_dyn_hello\0";
                        static A0: &[u8] = b"glibc_dyn_hello\0";
                ...
                if exit_code == 4 {
                    ...
                    syscall::debug_puts(b"Phase 172 dynamic glibc binary: FAILED (timeout)\n");
                } else {
                    syscall::debug_puts(b"Phase 172 dynamic glibc binary: FAILED (exit=");
                    let mut buf = [0u8; 10];
                    let mut val = exit_code as u32;
                    let mut i = 10;
                    if val == 0 { i -= 1; buf[i] = b'0'; }
