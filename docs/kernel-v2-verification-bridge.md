...
**Cross-partition delivery** (later milestone): when sender and receiver
are in different multikernel domains, `recv` may need to block and be
woken by an IPI.  That path is the SSG-3 interrupt-controller lift:
the Tessera `intc_weak_broadcast.v` results
(`bc_machine_ipi_step_via_intc`) already prove the ghost step for IPI
delivery through the controller; the transport's wakeup spec composes
with those theorems.  Domain scoping uses the SSG-1 topology model.
